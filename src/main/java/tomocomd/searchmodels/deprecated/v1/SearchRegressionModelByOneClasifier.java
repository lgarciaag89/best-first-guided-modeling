...
import tomocomd.searchmodels.deprecated.RegressionModelInfo;
import tomocomd.searchmodels.deprecated.RegressionOptimizationParam;
...
public class SearchRegressionModelByOneClasifier extends ASearchModelByOneClasifier {
  ...
  public SearchRegressionModelByOneClasifier(
      long mId,
      String trainPath,
      String testPath,
      ...
      if (!externalTestPath.isEmpty()) {
        for (String nameExt : externalTestPath) {
          ...
          if (extHead.isEmpty()) {
            ...
            extHead = String.format(FORMATER, extHead, headPart);
          }
        }
      }
      if (extHead.isEmpty()) line = String.format("%s,desc%n", head);
      else line = String.format("%s,%s,desc%n", head, extHead);
    }
    line += res;
  ...
  private String buildLineInfoModel(
    ...
    if (!resExternal.isEmpty()) {
      res = String.format(FORMATER, res, resExternal);
    }

    res = String.format(FORMATER, res, mdNames.toString().replace(",", " "));
