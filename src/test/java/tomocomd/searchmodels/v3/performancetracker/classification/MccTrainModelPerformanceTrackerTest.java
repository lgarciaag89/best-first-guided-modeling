...
class MccTrainModelPerformanceTrackerTest {

  MccTrainModelPerformanceTracker tracker = new MccTrainModelPerformanceTracker();

  @Test
  void testIsClassification() {
    assertTrue(tracker.isClassification());
  }

  @Test
  void testGetMetricType() {
    assertEquals(MetricType.MCC_TRAIN, tracker.getMetricType());
  }

  @Test
  void buildAndGetClassifierName() {
    List<ClassifierNameEnum> classifierNameList =
        BuildClassifierList.getClassifierNameList(new String[] {"all"}, true);
  ...
  Instances getTrain() {
    ...
    attributes.add(new Attribute("class", Arrays.asList("pos", "neg")));
